...
#define color_1 (Color){233, 219, 255, 255}
#define color_2 (Color){143, 74, 255, 255}
...
void draw_color_display(int active_color) {
    ...
    switch (active_color) {
        case 2:
            active_truecolor = color_2;
            break;
        case 3:
            active_truecolor = color_3;
            break;
        default:
            active_truecolor = color_1;
            break;
    }
    ...
    for (int i = 0; i < grid_h; i++) {
        for (int j = 0; j < grid_w; j++) {
            ...
            if (b->tab[j * grid_h + i] == 1)
                DrawRectangle(start_y + j * px, start_x + i * px, cell_size, cell_size,
                              color_1);
            if (b->tab[j * grid_h + i] == 2)
                DrawRectangle(start_y + j * px, start_x + i * px, cell_size, cell_size,
                              color_2);
            if (b->tab[j * grid_h + i] == 3)
                DrawRectangle(start_y + j * px, start_x + i * px, cell_size, cell_size,
                              color_3);
...
int neighbors(Board *b, int n, int x, int y) {
    ...
    int count = 0;
    if (x > 0 && b->tab[y * grid_h + (x - 1)] == n)
        count++;
    if (x < grid_h - 1 && b->tab[y * grid_h + (x + 1)] == n)
        count++;
    if (y > 0 && b->tab[(y - 1) * grid_h + x] == n)
        count++;
    if (y < grid_w - 1 && b->tab[(y + 1) * grid_h + x] == n)
        count++;
    if (x > 0 && y > 0 && b->tab[(y - 1) * grid_h + (x - 1)] == n)
        count++;
    if (x > 0 && y < grid_w - 1 && b->tab[(y + 1) * grid_h + (x - 1)] == n)
        count++;
    if (x < grid_h - 1 && y > 0 && b->tab[(y - 1) * grid_h + (x + 1)] == n)
        count++;
    if (x < grid_h - 1 && y < grid_w - 1 &&
        b->tab[(y + 1) * grid_h + (x + 1)] == n)
        count++;
    return count;
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
            ...
            int mx = GetMouseX();
            int my = GetMouseY();
            if ((mx > start_y) && (my > start_x) && (mx < w - start_y) &&
                (my < h - start_x)) {
                b->tab[grid_h * ((mx - start_y) / px) + ((my - start_x) / px)] =
                    active_color;
            }
        }
